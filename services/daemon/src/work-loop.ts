...
/** Runs one machine for one claimed run. The machine itself lives behind this. */
export type RunExecutor = (run: ClaimedRun) => Promise<RunEnd>;
...
export async function runWorkLoop(options: WorkLoopOptions, signal: AbortSignal): Promise<void> {
	const { orchestrator, nodeId, execute, logger, pollMs, sleep } = options;
	let failures = 0;
