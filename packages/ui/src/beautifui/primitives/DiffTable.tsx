...
export default function DiffTable({ rows = ROWS }: { rows?: DiffRow[]; variant?: string } = {}) {
		...
		<div className="w-full max-w-95">
			<div className="relative overflow-hidden rounded-card bg-surface shadow-card">
				<div className="primitive-card-bar flex items-center justify-between border-b border-line">
					...
					{settled && !accepted && (
						<span className="text-[11px] text-ink-3">Click changed rows to toggle</span>
					)}
				</div>
								...
								>
									...
									>
										<span className="flex items-center justify-between gap-2">
											...
											{row.removed && settled && (
												<IncludedMark included={edits[row.key]} tone="red" />
											)}
										</span>
									</td>
								</tr>
								...
								>
									<div className="overflow-hidden">
										...
										>
											<span
												className="primitive-table-cell text-[13px] font-medium tabular-nums transition-colors duration-200"
												style={{ color: edits.pistachio ? "var(--green)" : "var(--ink-3)" }}
											>
												Pistachio
											</span>
											...
											<span
												className="primitive-table-cell text-[13px] transition-colors duration-200"
												style={{ color: edits.pistachio ? "var(--green)" : "var(--ink-3)" }}
											>
												<span className="flex items-center justify-between gap-2">
